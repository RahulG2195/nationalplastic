...
const WishlistPage1 = () => {
    ...
    const handleDeleteSuccess = async (product_id) => {
        ...
        try {
            const response = await axios.delete(`http://localhost:3000/api/Wishlist`, { data: { product_id } });
            setWishlistItems(prevItems => prevItems.filter(item => item.product_id !== product_id))
        }
        catch (error) {
