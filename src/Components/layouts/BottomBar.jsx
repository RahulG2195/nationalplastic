  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setPreEventChair(allproducts.filter(products => products.category_id == 13));
        setWithoutArm(allproducts.filter(products => products.category_id == 14));
        setPrechair(allproducts.filter(products => products.category_id == 15));
        setPopularchair(allproducts.filter(products => products.category_id == 16));
        setCabinet(allproducts.filter(products => products.category_id == 17));
        setBabychair(allproducts.filter(products => products.category_id == 18));
    ...
    <div className="row px-4 py-2 d-flex align-items-center bottom_nav position-relative mainrow">
      <div className="col second ">
        ...
        <div className='ulCont SecondDrop mx-4 p-3'>
          <p className='text-start fw-bold dropHeading '>Premium Event Chairs</p>
          <div className='d-flex flex-row gap-5'>
            {chunkArray(preEventChair, 10).map((chunk, columnIndex) => (
              <div key={columnIndex} className="column">
                {chunk.map((product, index) => (
                  <p key={index}>
                    <Link  onClick={() => handleOnClick(product.seo_url)} className="nav-link" href={`/ProductDetail/${product.seo_url}`}>
