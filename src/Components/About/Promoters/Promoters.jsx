            ...
            {teamMembers.map((member, index) => (
              <div key={member.id}>
                <div className="team-members-container">
                  <div className={`row team-member align-items-center ${index % 2 === 0 ? "left-image" : "right-image"}`}>
                    <div className={`col-md-4 ${index % 2 === 0 ? "order-md-1 order-xs-1" : "order-md-2 order-xs-2"}`}>
                      <Team image={member.image_url} />
                    </div>
                    <div className={`col-md-8 ${index % 2 === 0 ? "order-2" : "order-1"}`}>
                      <div className="team-detail">
                        <h3 className="team-title">{member.name}</h3>
                        <h4 className="team-designation">{member.designation}</h4>
