...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({ message: "OutcomeAGM added successfully" }, { status: 201 });
  } catch (error) {
...
export async function PUT(request) {
  ...
  try {
    ...
    return NextResponse.json({ message: "OutcomeAGM updated successfully" }, { status: 200 });
  } catch (error) {
    console.error('Database query error:', error);
    return NextResponse.json({ message: "Internal server error" }, { status: 500 });
  }
...
export async function GET() {
  try {
    const outcomeAGMData = await query({
      query: `SELECT * FROM OutcomeAGM ORDER BY id`,
    });
