...
export async function GET(request) {
  try {
    const products = await query({
      query:
        "SELECT * FROM products WHERE prod_status = 1 GROUP BY product_name order by product_id",
      values: [],
    });

    const prod_clr = await query({
      query:
        "Select product_id, product_name, color, color_code FROM products WHERE prod_status = 1 order by product_id",
      values: [],
    });

    const limitProd = await query({
      query:
        "SELECT * FROM products WHERE prod_status = 1  GROUP BY product_name limit 12",
      values: [],
    });
